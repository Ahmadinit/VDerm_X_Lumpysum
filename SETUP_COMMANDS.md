...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd 'backend'; .\.venv\Scripts\Activate.ps1; & `"$NODE18_PATH\npm.cmd`" run start:dev"
```
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd 'VDerm-X'; & `"$NODE18_PATH\npx.cmd`" expo start --tunnel"
```
